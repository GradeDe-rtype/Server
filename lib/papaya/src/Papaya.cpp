...
Papaya::Papaya(std::string path, std::string name) {
  ...
  if (!file.is_open())
    throw PapayaError("The papaya you are trying to open does not exist",
                      "Papaya constructor");
  std::string line;
...
bool Papaya::hasData(const std::string &refKey, const std::string &refValue,
...
bool Papaya::hasLine(const std::string &refKey,
...
Papaya::getDatas() const {
  return _datas;
}

std::vector<std::string> Papaya::getLine(const std::string &key,
...
std::string Papaya::getData(const std::string &refKey,
...
void Papaya::addLine(const std::vector<std::string> &data) {
  if (data.size() != _keys.size())
    throw PapayaError("The data you are trying to add does not match the keys",
                      "Papaya add");
  std::unordered_map<std::string, std::string> map;
...
void Papaya::removeData(const std::string &refKey, const std::string &refValue,
...
void Papaya::removeLine(const std::string &refKey,
                        const std::string &refValue) {
  _checkKeys({refKey}, "Papaya::removeLine");
  _datas.erase(
      std::remove_if(
          _datas.begin(), _datas.end(),
          [refKey,
           refValue](const std::unordered_map<std::string, std::string> &data) {
            return data.at(refKey) == refValue;
          }),
      _datas.end());
}

void Papaya::updateData(const std::string &refKey, const std::string &refValue,
...
void Papaya::updateLine(const std::string &refKey, const std::string &refValue,
