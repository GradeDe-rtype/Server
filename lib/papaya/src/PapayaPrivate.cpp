...
void Papaya::_loadPapaya(const std::vector<std::string> &lines) {
  if (lines.size() < 1)
    throw PapayaError("The papaya you are trying to open is empty",
                      "Papaya constructor");
...
void Papaya::_loadPapayaKeys(const std::string &line) {
  ...
  if (_keys.size() < 1)
    throw PapayaError("The papaya you are trying to open has no keys",
                      "Papaya constructor");
}
...
void Papaya::_checkKeys(const std::vector<std::string> &keys,
