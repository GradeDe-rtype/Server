...
std::vector<std::string> rfcArgParser::ParseArray(std::string str) {
  ...
  if (tmp.size() == 0)
    throw rfcArgParser::Error(
        "Invalid array format, missing \"[]\" or empty string",
        "rfcArgParser::ParseArray");
...
rfcArgParser::ParseObject(std::string str) {
  std::unordered_map<std::string, std::string> result;
  std::string tmp = _keep(str, '{', '}');
  if (tmp.size() == 0)
    throw rfcArgParser::Error(
        "Invalid object format, missing \"{}\" or empty string",
        "rfcArgParser::ParseObject");
...
std::string
rfcArgParser::CreateObject(std::unordered_map<std::string, std::string> obj) {
  if (obj.size() == 0)
    throw rfcArgParser::Error("Invalid object format, empty object",
                              "rfcArgParser::CreateObject");
  std::string result = "{";
