...
Parsing::Parsing(int argc, char **argv) {
  ...
  if (argc != 3)
    throw ParsingError("Invalid number of arguments. Got " +
                       std::to_string(argc) + " expected 5.");
...
void Parsing::_parseArgPort(int argc, char **argv, int i) {
  ...
  if (!Utils::isNumber(argv[i + 1]))
    throw ParsingError("Invalid argument for \"-p\": \"" +
                       std::string(argv[i + 1]) + "\". Expect a number.");
