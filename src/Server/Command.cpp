...
Command::Command(TCP &tcp) : tcp_(tcp) {
  commands_["start"] = [this](const int client_id, const std::string &args) {
    start(client_id, args);
  };
  commands_["send"] = [this](const int client_id, const std::string &args) {
    send(client_id, args);
  };
  commands_["stop"] = [this](const int client_id, const std::string &args) {
    stop(client_id, args);
  };
  commands_["broadcast"] = [this](const int client_id,
                                  const std::string &args) {
    broadcast(client_id, args);
  };
  commands_["position"] = [this](const int client_id, const std::string &args) {
    position(client_id, args);
  };
}
