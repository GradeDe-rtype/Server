      ...
      *socket, [this, socket](const boost::system::error_code &error) {
        if (!error) {
          ...
          std::string welcome_message =
              "Welcome, Client " + std::to_string(new_player.getId()) + "!\n";
          boost::asio::async_write(
              *socket, boost::asio::buffer(welcome_message),
          ...
          strncpy(packet.args, rfcArgParser::CreateObject(data).c_str(),
                  sizeof(packet.args) - 1);
...
void TCP::start_read(Player player) {
  ...
  boost::asio::async_read(
      *socket, boost::asio::buffer(*buffer),
      [this, client_id, buffer, player](const boost::system::error_code &error,
                                        std::size_t length) {
        if (!error) {
          DataPacket packet{};
          std::memcpy(&packet, buffer->data(), length);
          std::cout << "Client " << client_id
                    << " sent a packet with command: " << packet.command
                    << "\n";
    ...
    if (id == receiver_id) {
      ...
      boost::asio::async_write(
          *socket, boost::asio::buffer(&data, sizeof(data)),
          [client_id, receiver_id](const boost::system::error_code &ec,
...
void TCP::send_broadcast(DataPacket data,
                         const std::vector<int> &excluded_clients) {
  for (const auto &player : players_) {
    const int id = player.getId();
    const auto socket = player.getSocket();
    if (std::find(excluded_clients.begin(), excluded_clients.end(), id) !=
        excluded_clients.end())
      continue;
    boost::asio::async_write(
        *socket, boost::asio::buffer(&data, sizeof(data)),
        [](const boost::system::error_code &ec, std::size_t bytes_transferred) {
          if (!ec) {
            std::cout << "Broadcast sent, bytes: " << bytes_transferred
                      << std::endl;
          } else {
            std::cerr << "Error sending broadcast: " << ec.message()
                      << std::endl;
          }
        });
    return;
